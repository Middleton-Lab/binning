...
bin.running <- function(whldat,
                        bin.size = 20,
                        first.bin.size,          # Currently = bin.size only
                        bins.out = 'ALL',
                        bin.start = 1,
...
if (missing(first.bin.size)) first.bin.size <- bin.size
...
if (bin.start != 1){
    if (backward){
        whldat.zero <- whldat.str[1:(bin.start-1),]
        whldat.zero <- whldat.zero[order(whldat.zero$interval, decreasing = TRUE),]
        # xx Probably should give a warning message here:
        #   Bin numbers are the bins counting down from the specified interval
    } else { # default is to bin going forward
...
if (n.bins < bins.out){
    stop("bins.out is too large. Not enough bins in data file.", call. = FALSE)
    }
...
hrminute <- strptime(paste(hr, minute), format = "%H %M")
whldat.zero <- whldat.zero[,-(1:2)]

# Aggregate hours and minutes.
running.time <- aggregate(
    list(hrminute = hrminute),
    by = list(bin = minbin),
    FUN = max)
# Convert to POSIX
running.time <- running.time[,2]
class(running.time) <- "POSIXct"
...
running <- cbind(whlnum, t.running, t.maxint, t.intsum, t.rpm)
} else {
# FIRST BIN != BIN SIZE
stop('Kevin hasn\'t implemented this yet', call. = FALSE)
}

# Return aggregated data
list(running = running, bins.out = bins.out, running.time = running.time)
